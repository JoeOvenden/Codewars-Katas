...
def choose_best_sum(t, k, ls):
    distance_combinations = combinations(ls, k)
    best_distance = -1
    for combination in distance_combinations:
        distance = sum(combination)
        if distance > best_distance and distance <= t:
            best_distance = distance
    return None if best_distance == -1 else best_distance